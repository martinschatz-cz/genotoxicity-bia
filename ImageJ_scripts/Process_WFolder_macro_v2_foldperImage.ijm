...
run("Close All");
run("Clear Results");
run("Text Window...", "name=[Info] width=90 height=16");

ver = "2.0"
print("[Info]","Version: " + ver + ", last edit 10.03.2023" +"\n");

print("[Info]","ImageJ version: " + IJ.getFullVersion +"\n");
run("Bio-Formats Macro Extensions");
Ext.getVersionNumber(version)
print("[Info]","Bio-formats version: " + version +"\n");

if (IJ.getFullVersion!="1.53t99") {
	print("[Info]","!!Alert!!" +"\n");
	print("[Info]","Tested ImageJ version: 1.53t99, current:" + IJ.getFullVersion +"\n");
}
...
for (i = 0; i < lengthOf(wells); i++) {
	...
	print("[Info]","Input directory: " + input +"\n");
	print("[Info]","File suffix: " + suffix +"\n");
	 ...
	 */
	setBatchMode(true);
	print("Well;Name;Position;Area;Mean;MoA");
	processFolder(input, suffix);
	setBatchMode(false);
	
	selectWindow("Log");
	saveAs("Text", resDir + File.separator + w_name + "_Log"+".csv");
	...
	for (i = 0; i < list.length; i++) {
		if(endsWith(list[i], suffix))
			if (indexOf(list[i], "DAPI") >= 0) {
				///// check image /////
				// open DAPI image
				print("[Info]","_________________" +"\n");
				print("[Info]","Processing: " + input + File.separator + list[i] +"\n");
...
///// Cy3 image /////
	...
	//print(pos[1]);
	if (num_results>4) {
		print(w_name + ";" + name + ";" + pos[1] + ";" + Table.get("Area", 0) + ";" + Table.get("Mean", 0) + ";" + Table.get("Mean", 0)/Table.get("Area", 0));
	}
