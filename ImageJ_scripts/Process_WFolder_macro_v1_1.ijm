...
As the next step, we applied "Otsu" auto threshold on image_2 smoothed with Gaussian blur (sigma=2.0) and got a new image mask out, image_3. The threshold values are saved and used on all DAPI images from the same well.
...
#@ String (label = "Well names (div=',')", value = "C2,C4") we
#@ String (label = "Nuclei label", value = "DAPI") nuclei_label
#@ String (label = "Primary label", value = "Cy3") prim_label
...
function processFolder(input, suffix) {
	...
	for (i = 0; i < list.length; i++) {
		if(endsWith(list[i], suffix))
			if (indexOf(list[i], nuclei_label) >= 0) {
				///// check image /////
				// open Nuclei label image
				...
				print("Nuclei label Size: " + dwidth + "x"+dheight);

				print("Apply Gaussian Blur, sigma=2.0");
...
///// Nuclei label image /////
	...
	//blur segmentation image to get smoother results
	print("Apply Gaussian Blur, sigma=2.0");
	run("Gaussian Blur...", "sigma=2");
	...
	//apply waterhsed to disconenct nuclei
	run("Watershed");
	...
	name=replace(file, nuclei_label, ""); //old //name = split(file,"DAPI");

	Cy3title=prim_label+name;
